    ...
    protected void handleLogin(ActionEvent event) {
        try {
            ...
            if (user != null && user.getPassword().equals(password)) {
                ...
                if ("diksha.gaonkar098@gmail.com".equals(email)) {
                    user.setRole("admin");
                } else {
                    user.setRole("user");
                }
                ...
                // Get role and navigate to appropriate view
                String role = user.getRole();
                Stage stage = (Stage) ((Button) event.getSource()).getScene().getWindow();
                URL location;
                
                if ("admin".equals(role)) {
                    location = getClass().getResource("/views/AdminView.fxml");
                    if (location == null) {
                        throw new Exception("Cannot find AdminView.fxml");
                    }
                } else {
                    location = getClass().getResource("/views/LoginView.fxml");
            ...
            showAlert(Alert.AlertType.ERROR, "System Error", "Login failed: " + e.getMessage());
            e.printStackTrace();
        }
    }
